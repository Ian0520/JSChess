...
example : 
1101 = 13 ==> 白棋能短入堡 黑棋兩邊能入堡
0101 = 9  ==> 白、黑棋都能短入堡
用bitwise判定(if (1101 & WKCA) != 0 .....)
...
function CheckBoard(){
    ...
    for(t_piece = PIECES.wp; t_piece <= PIECES.bK; ++t_piece){ //checking if pieces on gameboard and on piece list the same
    ...
    for(sq64 = 0; sq64 < 64; ++sq64){ //adding up value of pieces and pieces number
        sq120 = SQ120(sq64);
    ...
    for(t_piece = PIECES.wP; t_piece <= PIECES.bK; ++t_piece){ //checking pieces number
    ...
    if(t_material[COLORS.WHITE] != GameBoard.material[COLORS.WHITE] ||
         t_material[COLORS.BLACK] != GameBoard.material[COLORS.BLACK]){//checking materials of each side
            console.error('Error t_material');
            return BOOL.FALSE;
    }
    
    if(GameBoard.side != COLORS.WHITE && GameBoard.side != COLORS.BLACK){//checking side
