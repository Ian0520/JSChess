...
/*$("#FlipBoardButton").click( function(){
    var file, rank, sq, pce;
    var Temp_PceList = new Array(64)
    for(rank = RANKS.RANK_1; rank <= RANKS.RANK_8; rank++){
        for(file = FILES.FILE_A; file <= FILES.FILE_H; file++){
            sq = FR2SQ(file,rank);
            Temp_PceList[FR2SQ(7-file, 7-rank)] = GameBoard.pieces[sq];
            RemoveGUIPiece(sq);
            ClearPiece(sq);
        }
    }
    for(rank = RANKS.RANK_1; rank <= RANKS.RANK_8; rank++){
        for(file = FILES.FILE_A; file <= FILES.FILE_H; file++){
            sq = FR2SQ(file,rank);
            pce = Temp_PceList[sq];
            if(Temp_PceList[sq] != PIECES.EMPTY){
               AddGUIPiece(sq, pce);
                AddPiece(sq, pce); 
            }
        }
...
function ClickedSquares(pageX, pageY){
...
$(document).on('click','.Piece', function (e){
    ...
    if(UserMove.from == SQUARES.NO_SQ){
        UserMove.from = ClickedSquares(e.pageX, e.pageY);
    } else {
        UserMove.to = ClickedSquares(e.pageX, e.pageY);
    }
...
function CheckResult(){
    ...
    if(InCheck == BOOL.TRUE){
        if(GameBoard.side == COLORS.WHITE){
            $("#GameStatus").text("Game Over {Black Wins Via Checkmate}");
            return BOOL.TRUE;
        } else{
            $("#GameStatus").text("Game Over {White Wins Via Checkmate}");
            return BOOL.TRUE;
        }
